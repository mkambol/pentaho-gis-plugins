...
@Step ( id = "GisFileInput", image = "com/atolcd/pentaho/di/ui/trans/steps/images/GisFileInput.png", name =
  "GisFileInput.Shell.Name", description = "GisFileInput.Shell.Description", categoryDescription = "GisFileInput"
  + ".Shell.CategoryDescription", i18nPackageName = "com.atolcd.pentaho.di.trans.steps.gisfileinput" )
public class GisFileInputMeta extends BaseStepMeta implements StepMetaInterface {
  ...
  public GisFileInputMeta() {
    ...
    shpDef
      .addParameterDef( "FORCE_TO_2D", ValueMeta.TYPE_BOOLEAN, true, Arrays.asList( new String[] { "TRUE", "FALSE" } ),
        "TRUE" );
    shpDef.addParameterDef( "FORCE_TO_MULTIGEOMETRY", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    this.inputFormatDefs.put( "ESRI_SHP", shpDef );

    GisInputFormatDef geojsonDef =
      new GisInputFormatDef( "GEOJSON", new String[] { "*.geojson;*.GEOJSON", "*.json;*.JSON" },
        new String[] { "*.geojson", "*.json" } );
    geojsonDef.addParameterDef( "FORCE_TO_MULTIGEOMETRY", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    ...
    mapinfoDef.addParameterDef( "FORCE_TO_MULTIGEOMETRY", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    this.inputFormatDefs.put( "MAPINFO_MIF", mapinfoDef );

    // SpatialLite
    GisInputFormatDef sqlLiteDef =
      new GisInputFormatDef( "SPATIALITE", new String[] { "*.db;*.DB", "*.sqlite;*.SQLITE" },
    ...
    dxfDef.addParameterDef( "FORCE_TO_MULTIGEOMETRY", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    dxfDef.addParameterDef( "CIRCLE_AS_POLYGON", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    dxfDef.addParameterDef( "ELLIPSE_AS_POLYGON", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
    dxfDef.addParameterDef( "LINE_AS_POLYGON", ValueMeta.TYPE_BOOLEAN, true,
      Arrays.asList( new String[] { "TRUE", "FALSE" } ), "FALSE" );
  ...
  public void getFields( RowMetaInterface r, String origin, RowMetaInterface[] info, StepMeta nextStep,
    ...
    try {
      ...
      if ( inputFormat.equalsIgnoreCase( "ESRI_SHP" ) ) {
        fileReader = new ShapefileReader( space.environmentSubstitute( inputFileName ),
          space.environmentSubstitute( geometryFieldName ), charset.displayName() );
      } else if ( inputFormat.equalsIgnoreCase( "GEOJSON" ) ) {
        fileReader = new GeoJSONReader( space.environmentSubstitute( inputFileName ),
          space.environmentSubstitute( geometryFieldName ), charset.displayName() );
      } else if ( inputFormat.equalsIgnoreCase( "MAPINFO_MIF" ) ) {
        fileReader = new MapInfoReader( space.environmentSubstitute( inputFileName ),
          space.environmentSubstitute( geometryFieldName ), charset.displayName() );
        ...
        fileReader =
          new SpatialiteReader( space.environmentSubstitute( inputFileName ), tableName, charset.displayName() );
      } else if ( inputFormat.equalsIgnoreCase( "DXF" ) ) {
        fileReader =
          new DXFReader( space.environmentSubstitute( inputFileName ), space.environmentSubstitute( geometryFieldName ),
            charset.displayName(), false, false, false );
      }
